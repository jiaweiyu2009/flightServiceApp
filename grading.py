...
class TestFlightService(unittest.TestCase):
    ...
    def test_non_concurrency(self):
        ...
        for testcase in non_concurrent_testcases:
            ...
            users = parse_testcase(os.path.join(
                "testcases/non_concurrent/", testcase))
            counter += 1
            passed = True
            for user in users:
                output = user.call()
                if(output != user.results[0]):
                    passed = False
                    print(
                        "testcase {} fails, score for non_concurrent test {}/{}".format(testcase, score, counter))
                    print("your output for testcase {} is:\n{}".format(
                        testcase, output))
            if(passed):
                score += 1
                print(
                    "testcase {} passes, score for non_concurrent test {}/{}".format(testcase, score, counter))
            ...
            with ProcessPoolExecutor(max_workers=3) as executor:
                ...
                for i in range(5):
                    ...
                    if(passed == True):
                        score += 1
                        print(
                            "testcase {} passes, score for concurrent tests so far: {}/{}".format(testcase, score, counter))
                    else:
                        print(
                            "testcase {} fails, score for concurrent tests so far: {}/{}".format(testcase, score, counter))
                        print("your output for testcase {}\n\n user1 :\n {}\n\n user2 :\n {}\n\n".format(
                            testcase, futures[0].result().strip(), futures[1].result().strip()))
